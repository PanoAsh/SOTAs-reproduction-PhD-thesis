...
from dataloaders import sbd, pascal, davis, cityscapes_processed
...
class DELSE(object):
    def __init__(self, args):
        ...
        self.composed_transforms_tr = transforms.Compose([
            ...
            tr.FixedResize(resolutions={'crop_image': self.resolution, 'crop_gt': self.resolution}),
        ...
        elif self.args.dataset == 'cityscapes-processed':
        ...
        for ii, sample_batched in enumerate(self.trainloader):
            ...
            if not self.args.e2e and epoch < self.args.pretrain_epoch:
                ...
                phi_T_loss = vector_field_loss(energy, vfs, sdts) \
                             + LSE_output_loss(phi_T, gts, sdts, self.args.epsilon, dt_max=self.args.dt_max)
    ...
    def test(self, epoch, _save=True):
        ...
        with torch.no_grad():
            ...
            for ii, sample_batched in enumerate(self.testloader):
                ...
                result = crop2fullmask(phi_T, bbox, gt, zero_pad=self.args.zero_pad_crop, relax=self.args.relax_crop, bg_value=20)

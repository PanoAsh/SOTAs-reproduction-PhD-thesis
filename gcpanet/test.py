...
DATA_PATH = os.getcwd() + '/data/360ISOD-TE/'
MODEL_PATH = os.getcwd() + '/results/models/gcpanet_35.pth'
...
class Test(object):
    def __init__(self, Dataset, datapath, Network):
        ...
        self.loader = DataLoader(self.data, batch_size=1, shuffle=True, num_workers=1)
        ## network
        self.net    = Network(self.cfg)

        self.net.load_state_dict(torch.load(MODEL_PATH))  # for fine-tuning
    ...
    def save(self):
        with torch.no_grad():
            ...
            for image, mask, (H, W), name in self.loader:
                ...
                cv2.imwrite(os.path.join(SAVE_PATH, str(name)[2:-3]), out2)
                print(" {} images processed".format(count))
                count += 1


if __name__=='__main__':
    t =Test(dataset, DATA_PATH, GCPANet)
   # t.accuracy()
    t.save()

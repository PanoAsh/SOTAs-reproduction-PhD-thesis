...
class SalObjDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        fss = self.fs_loader(self.images[index][:70], self.images[index][75:], self.fss, image.size)
    ...
    def fs_loader(self, tr_pth, item, fs_list, img_size):
        fs_pth = tr_pth + '/FS/'
        m_list = []
        for fs in fs_list:
            temp = fs.split('_')
...
class test_dataset:
    def __init__(self, image_root,  gt_root):
        self.images = [image_root + f for f in os.listdir(image_root) if f.endswith('.jpg')]
        self.images = sorted(self.images)
        self.fs_pth = self.images[0][:-(len(self.images[0].split('/')[-1]))]
        for idx in range(len(self.images)):
            self.images[idx] = self.images[idx].split('/')[-1]
        self.gts = [gt_root + f for f in os.listdir(gt_root) if f.endswith('.jpg')
                    or f.endswith('.png')]
        self.gts = sorted(self.gts)
    ...
    def load_data(self):
        ...
        fss = self.fss_loader(self.fs_pth, self.images, name)
    ...
    def fss_loader(self, fs_pth, fs_list, item):
        m_list = []
        for fs in fs_list:
            temp = fs.split('_')
        ...
        for idx in range(len(idxs)):
            trg_fs_list.append(fs_list[idxs[idx]])
        imgs = []
        for idx in range(len(trg_fs_list)):
            img_pth = fs_pth + trg_fs_list[idx]
            with open(img_pth, 'rb') as f:
                img = Image.open(f)
                img.convert('RGB')
            imgs.append(img)

...
#build the model
model = BBSNet()
print_network(model, 'BBSNet')
if(opt.load is not None):
    model.load_state_dict(torch.load(opt.load))
    print('load model from ', opt.load)
model.cuda()

fs_branch, bbs_branch = [], []
for name, param in model.named_parameters():
    if 'resnet_fs' in name or 'pre_LSTM_L4_1' in name or 'pre_LSTM_L3_1' in name or 'pre_LSTM_L2' in name\
            or 'convLSTM' in name:
       # print(name)
        fs_branch.append(param)
    else:
        bbs_branch.append(param)
optimizer = torch.optim.Adam([{'params': fs_branch}, {'params': bbs_branch}], lr=opt.lr)

...
def cv_random_flip(img, label,depth, fss):
    ...
    if flip_flag == 1:
        ...
        fss = fss.transpose(Image.FLIP_LEFT_RIGHT)
    #top bottom flip
    # if flip_flag2==1:
...
def randomCrop(image, label,depth, fss):
    ...
    return image.crop(random_region), label.crop(random_region), depth.crop(random_region),  fss.crop(random_region)
def randomRotation(image, label, depth, fss):
    ...
    if random.random() > 0.8:
        ...
        fss = fss.rotate(random_angle, mode)
...
class SalObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root, depth_root, fs_root, trainsize):
        ...
        self.depths = [depth_root + f for f in os.listdir(depth_root) if f.endswith('.bmp')
                    or f.endswith('.png')]
        self.fss = [fs_root + f for f in os.listdir(fs_root) if f.endswith('.png')]
    ...
    def __getitem__(self, index):
        ...
        fss = self.binary_loader(self.fss[index])
        fss = fss.resize(image.size)
...
class test_dataset:
    ...
    def load_data(self):
        ...
        fss = self.binary_loader(self.fss[self.index])
        fss = self.fs_transform(fss).unsqueeze(0)

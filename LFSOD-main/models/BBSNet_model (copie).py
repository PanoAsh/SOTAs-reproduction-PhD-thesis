...
class BBSNet(nn.Module):
    def __init__(self, channel=32):
        ...
        #self.pre_LSTM_L4_1 = nn.Conv2d(2048, 64, kernel_size=1, stride=1, bias=True)
        #self.pre_LSTM_L3_1 = nn.Conv2d(1024, 64, kernel_size=1, stride=1, bias=True)
        #self.pre_LSTM_L2 = nn.Conv2d(512, 64, kernel_size=1, stride=1, bias=True)
        ...
        self.atten_fs_channel_0 = ChannelAttention(64*12)
        self.atten_fs_channel_1 = ChannelAttention(256*12)
        self.atten_fs_channel_2 = ChannelAttention(512*12)
        self.atten_fs_channel_3_1 = ChannelAttention(1024*12)
        self.atten_fs_channel_4_1 = ChannelAttention(2048*12)

        self.merge_fss_0 = nn.Conv2d(64 * 12, 64, kernel_size=1, stride=1, bias=True)
        self.merge_fss_1 = nn.Conv2d(256 * 12, 256, kernel_size=1, stride=1, bias=True)
        self.merge_fss_2 = nn.Conv2d(512 * 12, 512, kernel_size=1, stride=1, bias=True)
        self.merge_fss_3_1 = nn.Conv2d(1024 * 12, 1024, kernel_size=1, stride=1, bias=True)
        self.merge_fss_4_1 = nn.Conv2d(2048 * 12, 2048, kernel_size=1, stride=1, bias=True)
    ...
    def forward(self, x, x_depth, x_fss):
        ...
        for idx in range(12):
            x_fss[idx] = self.resnet_fs.conv1(x_fss[idx])
            x_fss[idx] = self.resnet_fs.bn1(x_fss[idx])
            x_fss[idx] = self.resnet_fs.relu(x_fss[idx])
            x_fss[idx] = self.resnet_fs.maxpool(x_fss[idx])

        # layer0 merge
        temp = x_depth.mul(self.atten_depth_channel_0(x_depth))
        temp = temp.mul(self.atten_depth_spatial_0(temp))
        x = x + temp
        temp_fss = torch.cat(x_fss, dim=1)
        temp_fss = temp_fss.mul(self.atten_fs_channel_0(temp_fss))
        temp_fss = temp_fss.mul(self.atten_fs_spatial_0(temp_fss))
        temp_fss = self.merge_fss_0(temp_fss)
        x = x + temp_fss
        # layer0 merge end
        ...
        for idx in range(12):
            x_fss[idx] = self.resnet_fs.layer1(x_fss[idx])
        ...
        temp_fss = self.merge_fss_1(temp_fss)
        x1 = x1 + temp_fss
        # layer1 merge end
        ...
        for idx in range(12):
            x_fss[idx] = self.resnet_fs.layer2(x_fss[idx])
        ...
        temp_fss = self.merge_fss_2(temp_fss)
        x2 = x2 + temp_fss
        # layer2 merge end
        ...
        for idx in range(12):
            x_fss[idx] = self.resnet_fs.layer3_1(x_fss[idx])
        ...
        temp_fss = self.merge_fss_3_1(temp_fss)
        x3_1 = x3_1 + temp_fss
        # layer3_1 merge end
        ...
        for idx in range(12):
            x_fss[idx] = self.resnet_fs.layer4_1(x_fss[idx])
        ...
        temp_fss = self.merge_fss_4_1(temp_fss)
        x4_1 = x4_1 + temp_fss
        # layer4_1 merge end

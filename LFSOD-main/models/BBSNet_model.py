...
#from models.graph_reasoning import GloRe
...
class BBSNet(nn.Module):
    def __init__(self, channel=32):
        ...
        self.FS_conv_2 = nn.Conv3d(512, 512, (3, 1, 1), stride=(1, 1, 1), padding=(1, 0, 0))
        self.FS_bn_2 = nn.BatchNorm3d(512)
        self.FS_conv_3_1 = nn.Conv3d(1024, 1024, (3, 1, 1), stride=(1, 1, 1), padding=(1, 0, 0))
        self.FS_bn_3_1 = nn.BatchNorm3d(1024)
        self.FS_conv_4_1 = nn.Conv3d(2048, 2048, (3, 1, 1), stride=(1, 1, 1), padding=(1, 0, 0))
        self.FS_bn_4_1 = nn.BatchNorm3d(2048)
        self.FS_relu = nn.ReLU(True)
        self.FS_reshape = nn.Conv2d(12, 1, 1)
        ...
        self.FS_fusion = nn.Conv3d(12, 1, (1, 1, 1))
    ...
    def forward(self, x, x_depth, x_fss):
        ...
        temp_fss = x2_fss.unsqueeze(0).permute(0, 2, 1, 3, 4)
        temp_fss = self.FS_conv_2(temp_fss)
        temp_fss = self.FS_bn_2(temp_fss)
        temp_fss = self.FS_relu(temp_fss)

        temp_x = x2.mul(self.atten_x_channel_2(x2))
        temp_x = temp_x.mul(self.atten_x_spatial_2(temp_x))
        x2_fss = x2_fss + temp_fss.squeeze(0).permute(1, 0, 2, 3) + temp_x

        temp_fss = temp_fss.squeeze(0)
        temp_fss = self.FS_reshape(temp_fss)
        temp_fss = temp_fss.permute(1, 0, 2, 3)
        ...
        temp_fss = x3_1_fss.unsqueeze(0).permute(0, 2, 1, 3, 4)
        temp_fss = self.FS_conv_3_1(temp_fss)
        temp_fss = self.FS_bn_3_1(temp_fss)
        temp_fss = self.FS_relu(temp_fss)

        temp_x = x3_1.mul(self.atten_x_channel_3_1(x3_1))
        temp_x = temp_x.mul(self.atten_x_spatial_3_1(temp_x))
        x3_1_fss = x3_1_fss + temp_fss.squeeze(0).permute(1, 0, 2, 3) + temp_x
        ...
        temp_fss = x4_1_fss.unsqueeze(0).permute(0, 2, 1, 3, 4)
        temp_fss = self.FS_conv_4_1(temp_fss)
        temp_fss = self.FS_bn_4_1(temp_fss)
        temp_fss = self.FS_relu(temp_fss)

        temp_x = x4_1.mul(self.atten_x_channel_4_1(x4_1))
        temp_x = temp_x.mul(self.atten_x_spatial_4_1(temp_x))
        x4_1_fss = x4_1_fss + temp_fss.squeeze(0).permute(1, 0, 2, 3) + temp_x
        ...
        attention_map_fss = attention_map_fss.unsqueeze(0)
        attention_map_fss = self.FS_fusion(attention_map_fss)
        attention_map_fss = attention_map_fss.squeeze(0)

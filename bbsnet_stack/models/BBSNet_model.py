...
class BBSNet(nn.Module):
    ...
    def forward(self, x, x_depth, x_fss):
        ...
        #cv2.imwrite('D.png', y.squeeze(0).permute(1, 2, 0).cpu().data.numpy() * 255)
        #cv2.imwrite('F.png', x_fss.squeeze(0).permute(1, 2, 0).cpu().data.numpy() * 255)

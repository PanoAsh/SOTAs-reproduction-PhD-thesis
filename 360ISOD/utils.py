...
class common_prepro():
    ...
    def num_obj(self):
        objects = os.listdir(os.getcwd() + '/texts/')
        objects.sort(key=lambda x: x[:-4])
        objects_list = []

        for img in objects:
            obj_path = os.path.join(os.path.abspath(os.getcwd() + '/texts/'), img)
    ...
    def num_ins(self):
        ...
        class_list = Counter(flattened_instance_list)
        class_list = OrderedDict(class_list.most_common())
